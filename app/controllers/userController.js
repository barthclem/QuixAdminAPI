...
class UserController {
    ...
    userLogin (authMiddleware, req, res, next) {
        ...
        this.userService.loginUser(data).then((loginData) => {
            if (loginData) {
                        ...
                        .catch(error => {
                            ...
                            return res.status(HttpStatus.INTERNAL_SERVER_ERROR)
                                .send(responseFormatter(HttpStatus.INTERNAL_SERVER_ERROR, {
                                    message: 'please try again later as our system is experiencing a failure'
                                }));
                        });
                }
                ...
                roles.each(roleData => {
                    userRoles.add(roleData.attributes.role_title);
                });
                let exportRoles = [...userRoles];
    ...
    */
    checkToken (res) {
        return res.status(HttpStatus.OK)
            .send(responseFormatter(HttpStatus.OK, { message: 'token is valid' }));
    }
